...
EE_OPTIONS = {
    ...
    "hitbot": "Hitbot gripper",
    "suction_cup": "Galbot suction cup",
    "none": "No end effector",
}
EE_FILENAME_NAMES = {
    "hitbot": "hitbot_gripper",
}
COLLIDER_OPTIONS = ("simple", "sphere", "convex_hull", "convex_decomposition", "mesh")
...
def require_xacro_module():
    ...
    for site_packages in _ros_python_site_packages():
        ...
        sys.modules.pop("xacro", None)
        try:
            sys.path.remove(site_packages.as_posix())
        except ValueError:
            pass
...
def interactive_config(args: argparse.Namespace) -> argparse.Namespace:
    ee_options = [
        ...
        ("1", "Hitbot Gripper", "hitbot"),
        ("2", "Galbot Suction Cup", "suction_cup"),
        ("3", "None", "none"),
    ]
    collider_options = [
        ("0", "Simple [Recommended]", "simple"),
        ("1", "Sphere", "sphere"),
        ("2", "Convex Hull", "convex_hull"),
        ("3", "Convex Decomposition", "convex_decomposition"),
        ("4", "Original Mesh", "mesh"),
    ]
    camera_options = [
    ...
    args.ee = select_from_options(
        title="Please select the default end effector for both arms",
        options=ee_options,
    )
    args.left_ee = select_from_options(
        title="Please select the left arm end effector",
        options=[("0", "Use default end effector", "")] + ee_options,
    )
    args.right_ee = select_from_options(
        title="Please select the right arm end effector",
        options=[("0", "Use default end effector", "")] + ee_options,
    )
    args.collider = select_from_options(
        title="Please select the collision type",
        options=collider_options,
    )
    ...
    print("\nYou have selected:")
    print(f"Default EE: {args.ee}")
    print(f"Left EE: {args.left_ee or args.ee}")
    print(f"Right EE: {args.right_ee or args.ee}")
...
def default_output_name(
    *,
    ...
    left_name = EE_FILENAME_NAMES.get(left_ee, left_ee)
    right_name = EE_FILENAME_NAMES.get(right_ee, right_ee)
    name = f"{ROBOT_NAME}_left_{left_name}_right_{right_name}"
    if collider != "simple":
        name += f"_collision_{collider}"
    if arm_camera != "d405":
        name += f"_camera_{arm_camera}"
    return f"{name}.urdf"
...
def xacro_mappings(args: argparse.Namespace) -> tuple[dict[str, str], str, str]:
    ...
    validate_choice("collider", args.collider, COLLIDER_OPTIONS)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--collider",
        default="simple",
        choices=COLLIDER_OPTIONS,
        help="Collision geometry type passed as xacro collider.",
    )
    parser.add_argument(
        "--arm-camera",
        default="d405",
